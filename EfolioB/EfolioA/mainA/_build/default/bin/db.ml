...
(** Tenta converter uma string em inteiro, capturando falhas. *)
let safe_int_of_string s =
  try Ok (int_of_string s)
  with Failure _ -> Error ("Campo inteiro inválido: '"^s^"'")

(** Tenta converter uma string em float, capturando falhas. *)
let safe_float_of_string s =
  try Ok (float_of_string s)
  with Failure _ -> Error ("Campo float inválido: '"^s^"'")

(* Extrai um item de uma linha em formato Prolog *)
let parse_item (line : string) : (item, string) result =
  let re = Str.regexp
    "item(\\([0-9]+\\), '\\([^']+\\)', '\\([^']+\\)', '\\([^']+\\)', \\([0-9.]+\\), \\([0-9.]+\\), \\([0-9]+\\))."
  in
  if not (Str.string_match re line 0) then
    Error ("Linha não corresponde ao padrão de item: "^ line)
  else
    (* extração segura de cada grupo *)
    let g n = Str.matched_group n line in
    match safe_int_of_string   (g 1),
          Ok       (g 2),
          Ok       (g 3),
          Ok       (g 4),
          safe_float_of_string (g 5),
          safe_float_of_string (g 6),
          safe_int_of_string   (g 7) with
    | Ok id, Ok nome, Ok marca, Ok tipo,
      Ok custo, Ok preco, Ok qtd ->
        Ok { id; nome; marca; tipo; custo; preco; qtd }
    | Error e, _, _, _, _, _, _
    | _, Error e, _, _, _, _, _
    | _, _, Error e, _, _, _, _
    | _, _, _, Error e, _, _, _
...
(* Extrai um desconto de mão de obra de uma linha, agora usando lt(...) / gt(...) *)
let parse_discount_obra line : desconto_obra option =
  let re_lt = Str.regexp "desconto_mao_obra(lt(\\([0-9.]+\\)), \\([0-9.]+\\))." in
  let re_gt = Str.regexp "desconto_mao_obra(gt(\\([0-9.]+\\)), \\([0-9.]+\\))." in
...
let load_items () : item list =
  read_file "database.pl"
  |> List.map (fun line ->
      match parse_item line with
      | Ok it   -> Some it
      | Error e ->
          prerr_endline ("[load_items] "^e);
          None)
